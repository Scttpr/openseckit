...
for f in registre-traitement aipd lia mesures-securite; do
    curl -sf "$GITHUB_RAW/kit/comply/frameworks/rgpd/templates/core/${f}.md.j2" -o "$DIR/templates/core/${f}.md.j2"
done
for f in droits-personnes violation-donnees; do
    curl -sf "$GITHUB_RAW/kit/comply/frameworks/rgpd/templates/procedures/${f}.md.j2" -o "$DIR/templates/procedures/${f}.md.j2"
done
curl -sf "$GITHUB_RAW/kit/comply/frameworks/rgpd/templates/contracts/clause-sous-traitant.md.j2" -o "$DIR/templates/contracts/clause-sous-traitant.md.j2"
curl -sf "$GITHUB_RAW/kit/comply/frameworks/rgpd/templates/public/politique-confidentialite.md.j2" -o "$DIR/templates/public/politique-confidentialite.md.j2"
for f in assessment-summary export-report gap-report sub-processor-register; do
    curl -sf "$GITHUB_RAW/kit/comply/frameworks/rgpd/templates/${f}.md.j2" -o "$DIR/templates/${f}.md.j2"
done
echo "  ✓ templates"
