...
for f in index business product architecture data actors boundaries integrations controls tooling team operations gaps glossary supply_chain user-journeys; do
    ...
    curl -sf "$GITHUB_RAW/kit/discover/templates/data/${f}.yaml.j2" -o "$DIR/templates/data/${f}.yaml.j2"
done
echo "  ✓ templates/data"
...
for f in architecture developer onboarding operations product security; do
    ...
    curl -sf "$GITHUB_RAW/kit/discover/templates/outputs/${f}.md.j2" -o "$DIR/templates/outputs/${f}.md.j2"
done
echo "  ✓ templates/outputs"

# Prompts
for f in discover 01-product-context 02-architecture 03-domain-model 04-ecosystem 05-operations 06-synthesis 07-documentation; do
    mkdir -p "$DIR/prompts"
